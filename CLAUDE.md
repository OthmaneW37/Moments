...
### Backend (`backend/app/`)
- **`main.py`** — every route lives here (auth, events CRUD, photos/videos, follows (`/api/users/{u}/follow`, `/api/follow/requests`, `/api/follow/respond`, followers/following lists), feed, discover, reactions, comments, notifications, timeline, stats, memories, week-summary, recap, `/api/users/{username}` public profile, `/api/context/detail` work page + work discussion, `/api/search/users`, share (`/api/events/{id}/share` + public `/api/shared/{token}`), DM (`/api/conversations*`)). One flat FastAPI app.
...
- **A "moment" in the social sense = an event that has ≥1 photo/video.** Feed/Discover/Timeline/memories all filter on `JOIN photos`. The `photos` table stores both images and videos, discriminated by `media_type` ('photo'|'video'); the shared `<Media>` component renders `<video>` vs `<img>`. Uploads accept `IMAGE_EXT | VIDEO_EXT`; videos skip pixel auto-tagging.
- **Extra social surfaces (Phase 6):** work discussion threads (`work_comments`, keyed by `kind`+lowercased title, shown on ContextSheet), global account search (`/api/search/users`), public share links (`events.share_token`; the frontend routes `/s/<token>` to `SharedMoment`, an auth-free read-only page), and private DMs (`conversations`/`messages` tables; `Messages` component with a conversation list + thread; a `message` notification type; unread count polled in `App`).
