...
- **Feed vs Discover:** feed = me + accepted friends; discover = `visibility='public'` events from everyone *except* me and my friends, with same-city results ranked first. Both build cards through the shared `_enrich_moments()` helper in `main.py`.
- **Notifications** are written via `db.notify()` (silently skips notifying yourself). Types: `friend_request`, `friend_accept`, `like`, `comment`.
